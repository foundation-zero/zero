...
const props = withDefaults(defineProps<ManualValveProps>(), {
  orientation: MANUAL_VALVE_BASE_ORIENTATION,
});

const { orientation } = toRefs(props);

const rotation = useOrientation(orientation, MANUAL_VALVE_BASE_ORIENTATION);
</script>

<template>
  <svg
    data-slot="manual-valve"
    v-bind="createSizeAndViewbox(MANUAL_VALVE_WIDTH, MANUAL_VALVE_HEIGHT)"

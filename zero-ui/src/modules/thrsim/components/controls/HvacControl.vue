...
import { HvacExchangerSimulation } from "@/modules/thrsim/types";
...
const props = defineProps<{
  values: HvacExchangerSimulation;
...
<template>
  <div class="bg-background flex flex-col rounded-md border p-4 shadow-md">
    ...
    <div class="mt-6 grid grid-cols-2 gap-x-2">
      ...
      <div class="grid gap-1.5">
        <header class="text-2xs tracking-wide uppercase">
          {{ t("components.inputs.maximumTemperature.label") }}
        </header>
        <NumberField
          v-model="maximumTemperature.value.value"
          class="grow"
          :step="1"
          :min="0"
          :max="100"
    ...
    <Button
      :disabled="isSubmitting || (!heatFlow.isDirty.value && !maximumTemperature.isDirty.value)"
      class="mt-6"
      @click="submit"
    >
      <RiLoader2Line

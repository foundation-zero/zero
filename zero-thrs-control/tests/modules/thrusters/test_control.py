...
def test_recovery_temperature(
    ...
    for i in range(60):
        ...
        result = simulation.tick(control_values)
        assert controller_values.control_mode == ThrustersControlMode(mode="recovery")
        assert (
            result.sensor_values.thrusters_temperature_recovery.temperature.value
            == approx(
                control.parameters.recovery_temperature,
...
def test_flow_cooling(control: ThrustersControl, simulation: ThrustersSimulation):
    ...
    for i in range(60):
        control_values, controller_values = control.control(result.sensor_values)
        result = simulation.tick(control_values)
        assert controller_values.control_mode == ThrustersControlMode(mode="cooling")
...
def test_cooldown(control: ThrustersControl, simulation: ThrustersSimulation):
    ...
    assert controller_values.control_mode == ThrustersControlMode(mode="cooldown")
    while controller_values.control_mode == ThrustersControlMode(mode="cooldown"):
        control_values, controller_values = control.control(result.sensor_values)
        result = simulation.tick(control_values)
    ...
    assert controller_values.control_mode == ThrustersControlMode(mode="idle")

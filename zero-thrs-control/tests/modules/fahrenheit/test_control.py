...
async def test_state_mode_switches(control: FahrenheitControl, simulation: Simulation):
    # start with cooling demand and insufficient heat
    simulation._simulation_inputs.fahrenheit_available_hot_temperature.temperature.value = (
        control._parameters.fahrenheit_hot_minimum - 1
    )
    simulation._simulation_inputs.fahrenheit_available_cold_temperature.temperature.value = (
...
async def test_fahrenheit_cooling(control: FahrenheitControl, simulation: Simulation):
    result = await simulation.tick(control.initial().values)
...
async def test_waste_recovery(control: FahrenheitControl, simulation: Simulation):
    simulation._simulation_inputs.fahrenheit_boilers_supply.temperature.value = 20
...
async def test_waste_cooling():
    ...
    with Fmu(fahrenheit_path) as fmu:
        ...
        control = FahrenheitControl(parameters, simulation.time)

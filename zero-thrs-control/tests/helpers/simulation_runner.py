...
class SimulationTestRunner[
    ...
    def __init__(
        ...
        self._alarms = alarms
        self._control_values, self._controller_values = self._control.initial()
    ...
    def tick(
    ...
    ) -> tuple[SimulationResult | None, C, M]:
            ...
            collector.collect(  # TODO: fix the fmu key mapping here, this is just a quick fix to get the tests working
                {
                    ...
                    **flatten_model_values(
                        ...
                        build_fmu_key_mapping(
                            type(self._controller_values), fmu_only=False
                        ),
                    ),
                    **flatten_model_values(
        ...
        self._control_values, self._controller_values = self._control.control(
            result.sensor_values
        )
        ...
        return self.last_tick_result, self._control_values, self._controller_values
    ...
    def run_until(
        self,
        condition: Callable[[SimulationResult | None, C, M], bool],
        collector: Collector | None = None,
    ) -> tuple[SimulationResult | None, C, M]:
        while not condition(
            self.last_tick_result, self._control_values, self._controller_values
        ):
            self.tick(collector)
        return self.last_tick_result, self._control_values, self._controller_values

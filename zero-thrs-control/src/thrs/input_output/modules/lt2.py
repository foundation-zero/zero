from typing import Annotated
from thrs.input_output.base import (
    SimulationInputs,
    SimulationValues,
    ThrsValues,
    component_meta,
)
from thrs.input_output.definitions import control, sensor, simulation


class Lt2SensorValues(ThrsValues):
    lt2_pump_aft: Annotated[
        sensor.Pump, component_meta(yard_tag="50001020", component_type="pump")
    ]
    lt2_pump_ugrid: Annotated[
        sensor.Pump, component_meta(yard_tag="50001023", component_type="pump")
    ]
    lt2_pump_fwd: Annotated[
        sensor.Pump, component_meta(yard_tag="50001025", component_type="pump")
    ]
    lt2_temperature_aft4_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-05", component_type="temperature_sensor"),
    ]
    lt2_temperature_aft3_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-06", component_type="temperature_sensor"),
    ]
    lt2_temperature_aft2_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-07", component_type="temperature_sensor"),
    ]
    lt2_temperature_aft1_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-08", component_type="temperature_sensor"),
    ]
    lt2_temperature_ugrid2_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-09", component_type="temperature_sensor"),
    ]
    lt2_temperature_ugrid1_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-10", component_type="temperature_sensor"),
    ]
    lt2_temperature_fwd2_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-12", component_type="temperature_sensor"),
    ]
    lt2_temperature_fwd1_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-13", component_type="temperature_sensor"),
    ]
    lt2_temperature_aft_supply: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-15", component_type="temperature_sensor"),
    ]
    lt2_temperature_recovery_mix: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-17", component_type="temperature_sensor"),
    ]
    lt2_temperature_supply: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-18", component_type="temperature_sensor"),
    ]
    lt2_temperature_fwd_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-19", component_type="temperature_sensor"),
    ]
    lt2_temperature_aft_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-20", component_type="temperature_sensor"),
    ]
    lt2_temperature_recovery: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-52", component_type="temperature_sensor"),
    ]
    lt2_temperature_recovery_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-58", component_type="temperature_sensor"),
    ]
    lt2_temperature_fwd_supply: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-69", component_type="temperature_sensor"),
    ]
    lt2_temperature_ugrid_supply: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-70", component_type="temperature_sensor"),
    ]
    lt2_temperature_ugrid_return: Annotated[
        sensor.TemperatureSensor,
        component_meta(yard_tag="50001038-71", component_type="temperature_sensor"),
    ]
    lt2_mix_fwd: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001042-03", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_aft: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001043-02", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_ugrid: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001045-01", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_recovery: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001046-04", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_exchanger: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001046-05", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_flow_aft4: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-04", component_type="flow_sensor"),
    ]
    lt2_flow_aft3: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-05", component_type="flow_sensor"),
    ]
    lt2_flow_aft2: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-06", component_type="flow_sensor"),
    ]
    lt2_flow_aft1: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-07", component_type="flow_sensor"),
    ]
    lt2_flow_ugrid2: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-08", component_type="flow_sensor"),
    ]
    lt2_flow_ugrid1: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-09", component_type="flow_sensor"),
    ]
    lt2_flow_fwd2: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-11", component_type="flow_sensor"),
    ]
    lt2_flow_fwd1: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-12", component_type="flow_sensor"),
    ]
    lt2_flow_aft_return: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-25", component_type="flow_sensor"),
    ]
    lt2_flow_fwd_return: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001057-26", component_type="flow_sensor"),
    ]
    lt2_flow_recovery: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001058-04", component_type="flow_sensor"),
    ]
    lt2_flow_ugrid_return: Annotated[
        sensor.FlowSensor,
        component_meta(yard_tag="50001058-06", component_type="flow_sensor"),
    ]
    lt2_switch_aft4: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001068-01", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_aft3: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001068-02", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_aft2: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001068-03", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_aft1: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001068-04", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_fwd2: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001068-05", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_fwd1: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001068-06", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_ugrid2: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001069-02", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_ugrid1: Annotated[
        sensor.Valve,
        component_meta(
            yard_tag="50001069-03", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_pressure_aft: Annotated[
        sensor.PressureSensor,
        component_meta(yard_tag="50001097-07", component_type="pressure_sensor"),
    ]
    lt2_pressure_ugrid: Annotated[
        sensor.PressureSensor,
        component_meta(yard_tag="50001097-08", component_type="pressure_sensor"),
    ]
    lt2_pressure_fwd: Annotated[
        sensor.PressureSensor,
        component_meta(yard_tag="50001097-09", component_type="pressure_sensor"),
    ]
    lt2_brightloop_aft1: Annotated[
        sensor.Brightloop,
        component_meta(yard_tag="45002076", component_type="brightloop"),
    ]
    lt2_brightloop_aft2: Annotated[
        sensor.Brightloop,
        component_meta(yard_tag="45002075", component_type="brightloop"),
    ]
    lt2_brightloop_aft3: Annotated[
        sensor.Brightloop,
        component_meta(yard_tag="45002074", component_type="brightloop"),
    ]
    lt2_brightloop_aft4: Annotated[
        sensor.Brightloop,
        component_meta(yard_tag="45002073", component_type="brightloop"),
    ]
    lt2_brightloop_fwd1: Annotated[
        sensor.Brightloop,
        component_meta(yard_tag="45002078", component_type="brightloop"),
    ]
    lt2_brightloop_fwd2: Annotated[
        sensor.Brightloop,
        component_meta(yard_tag="45002077", component_type="brightloop"),
    ]
    lt2_ugrid1: Annotated[
        sensor.Ugrid,
        component_meta(yard_tag="45002082", component_type="ugrid"),
    ]
    lt2_ugrid2: Annotated[
        sensor.Ugrid,
        component_meta(yard_tag="45002081", component_type="ugrid"),
    ]


class Lt2ControlValues(ThrsValues):
    lt2_pump_aft: Annotated[
        control.Pump, component_meta(yard_tag="50001020", component_type="pump")
    ]
    lt2_pump_fwd: Annotated[
        control.Pump, component_meta(yard_tag="50001025", component_type="pump")
    ]
    lt2_pump_ugrid: Annotated[
        control.Pump, component_meta(yard_tag="50001023", component_type="pump")
    ]
    lt2_mix_aft: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001043-02", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_fwd: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001042-03", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_ugrid: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001045-01", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_recovery: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001046-04", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_mix_exchanger: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001046-05", component_type="valve", valve_type="mix"
        ),
    ]
    lt2_switch_aft4: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001068-01", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_aft3: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001068-02", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_aft2: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001068-03", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_aft1: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001068-04", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_fwd2: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001068-05", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_fwd1: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001068-06", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_ugrid2: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001069-02", component_type="valve", valve_type="switch"
        ),
    ]
    lt2_switch_ugrid1: Annotated[
        control.Valve,
        component_meta(
            yard_tag="50001069-03", component_type="valve", valve_type="switch"
        ),
    ]


class Lt2SimulationInputs(SimulationInputs):
    lt2_brightloop_fwd1: simulation.HeatSource
    lt2_brightloop_fwd2: simulation.HeatSource
    lt2_ugrid1: simulation.HeatSource
    lt2_ugrid2: simulation.HeatSource
    lt2_brightloop_aft1: simulation.HeatSource
    lt2_brightloop_aft2: simulation.HeatSource
    lt2_brightloop_aft3: simulation.HeatSource
    lt2_brightloop_aft4: simulation.HeatSource
    lt2_seawater_supply: simulation.Boundary
    lt2_boilers_supply: simulation.Boundary


class Lt2SimulationOutputs(SimulationValues):
    lt2_seawater_return: simulation.TemperatureBoundary
    lt2_boilers_return: simulation.TemperatureBoundary

<template>
  <article class="w-100 flex justify-around items-center">
    <div>
      <span class="inline-flex flex-col items-center justify-center">
        <span class="text-5xl md:portrait:text-7xl font-bold">18</span>
        <span class="text-md font-extralight">Inside</span>
      </span>
      <sup class="text-3xl md:portrait:text-5xl font-extralight">&deg;</sup>
    </div>

    <div>
      <span class="inline-flex flex-col items-center justify-center">
        <span class="text-5xl md:portrait:text-7xl font-bold">33</span>
        <span class="text-md font-extralight">Outside</span>
      </span>
      <sup class="text-3xl md:portrait:text-5xl font-extralight">&deg;</sup>
    </div>
  </article>
</template>

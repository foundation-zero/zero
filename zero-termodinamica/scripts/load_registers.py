...
def load_registers(path: Path, sheet_name: str, unit_id: int):
    ...
    if sheet_name == "Vent":
        ...
        df = df.with_columns(
            pl.col("address").cast(pl.Int32, strict=False).alias("address")
        )
    ...
    df = (
        ...
        .with_columns(
            pl.lit(unit_id).alias("unit_id"),
            pl.col("register_name").str.extract("REG_SPLIT_(\d+)").alias("reg_split"),
            pl.col("register_name")
            .str.extract("REG_SPLIT_\d+_READ_(.*)", 1)
            .str.replace(" ", "")
            .alias("variable_name"),

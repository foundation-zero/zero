    ...
    try:
        ...
        for address in PROBE_ADDRESSES:
            server.data_bank.set_holding_registers(
                address.register, float_to_lsw_registers(20.0)
            )
        temperatures = await TemperatureReader(
...
async def test_mqtt_switching(mqtt_client, mqtt_client2, modbus_settings):
    ...
    stub = Stub(
        server_mqtt, modbus_settings.modbus_server(), "test/topic", "$.path", 20.0
    )

    reader = RelaySwitchingTemperatureReader(
        modbus_settings.modbus_client(),
        client_mqtt,
        "test/topic",
        "$.path",
...
async def test_mqtt_sending(mqtt_client, mqtt_client2, mqtt_client3, modbus_settings):
    ...
    try:
        ...
        for reading in result.temperatures.values():
            assert reading == 20.0
    finally:
        await stub.disable_modbus()
        run_stub.cancel()

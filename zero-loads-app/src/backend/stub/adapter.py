...
class PCanAdapter:
    def __init__(self, localIP: str, localPort: int):
    ...
    async def read(self):
        ...
        self._decode_can_message(message)

    async def send(self, message: str):
        # convert message to bytes
        byte_message = message
        self.socket.sendto(byte_message, (self.localIP, self.localPort))

    @staticmethod
    def _decode_can_message(message):
        ...
        if CAN_MSG_FLAG & 0x20:
            logging.info("Bit Rate Switch aktive ")
        if CAN_MSG_FLAG & 0x10:
            logging.info("Frame use Extended Data Lenght ")
        ...
        logging.info("CAN ID: " + "0x" + "{:x}".format(CAN_ID_NUM) + " ", end="")

        # Get the DLC
        DLC = message[21]  # place of the DLC Information
        logging.info("CAN DLC: " + format(DLC) + " ", end="")

        # using CAN FD DLC is NOT Lenght - convert it first - if less / eq 8 - keep it as it is...!
        LEN = DLC_to_LEN[DLC]
        if (
            LEN > 8
        ):  # only if we use CAN-FD and receive a DLC >8 teh D>LC is not the Length
            logging.info("CAN Data Byte counter: " + format(LEN), end="")

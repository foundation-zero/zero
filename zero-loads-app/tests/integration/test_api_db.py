...
async def test_declarative_base_matches_db(settings: Settings, sessionmanager):
    async with sessionmanager.connect() as conn:
        ...
        def get_db_objects_and_columns(sync_conn, schema=schema):
            ...
            db_columns = {t: {col["name"] for col in inspector.get_columns(t, schema=schema)} for t in db_objects}
        ...
        assert model_tables <= db_objects, f"Missing tables: {model_tables - db_objects}"
        for table in model_tables:
            model_cols = set(Base.metadata.tables[table].columns.keys())
            assert model_cols == db_columns[table], f"Column mismatch {table}: {model_cols} != {db_columns[table]}"
